    ...
    if (operation === 'dashboard') {
      ...
      if (dashboard.total_requests > 0) {
        ...
        for (const model in dashboard.by_model) {
          ...
          const modelResponseTimes = modelStats.map(s => Number((s.data || s).response_time_ms) || 0).filter(t => t > 0);
          ...
          // 按模型计算缓存命中率
          const modelCached = modelStats.reduce((sum, s) => sum + (Number((s.data || s).cached_tokens) || 0), 0);
          const modelInput = modelStats.reduce((sum, s) => sum + (Number((s.data || s).input_tokens) || 0), 0);
