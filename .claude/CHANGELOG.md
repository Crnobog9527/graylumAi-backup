...
**React 18 StrictMode 双重渲染导致防重复机制失效**

React 18 在开发环境下会故意 mount → unmount → mount 组件两次，用于检测不纯副作用。
组件级的 `useRef` 在重新 mount 时会被重新初始化，导致 `autoSentRef.current` 从 true 变回 false。

```
Mount #1: autoSentRef = useRef(false) → 设为 true → 发送请求
卸载：组件被销毁
...
使用模块级变量代替组件级 `useRef`，因为模块级变量不会因组件重新创建而重置。

```javascript
// 模块级变量，不受 StrictMode 重渲染影响
let globalAutoSendTriggered = false;

export function useChatState() {
  // 组件卸载时延迟重置（让 StrictMode 快速重挂载保留状态）
  useEffect(() => {
    return () => {
      setTimeout(() => {
...
| 文件 | 变更说明 |
|------|----------|
| `useChatState.jsx` | 添加 `globalAutoSendTriggered` 模块级变量 |
| `useChatState.jsx` | 清理诊断日志，保留必要的防重复逻辑 |

### 经验教训

1. **StrictMode 影响 useRef**：组件重新 mount 时 useRef 会被重新初始化
2. **模块级变量更可靠**：对于跨渲染周期的状态追踪，模块级变量更稳定
3. **延迟重置技巧**：200ms 延迟可区分 StrictMode 快速重挂载和真正的页面切换

---

## 2026-01-13 (对话窗口隔离性失效 - 完整修复) 🔧

### 问题背景

延续 2026-01-12 的修复工作，发现问题比预期更复杂，涉及多层技术栈。

### 发现的问题链

| 序号 | 问题 | 根因 |
|------|------|------|
| 1 | 重复发送 2 次请求 | React setState 异步 + IME 输入法 |
| 2 | 新对话不显示在历史 | RLS "Creator Only" 不生效 |
| 3 | asServiceRole 查询失败 | Base44 asServiceRole 对 Read 也受 RLS 限制 |
| 4 | Update 操作被拒绝 | Base44 asServiceRole 对 Update 也受 RLS 限制 |

### 修复内容

**前端** (`src/components/hooks/useChatState.jsx`):
- 添加 `isStreamingRef` 同步检查，防止重复发送
- 添加 `e.isComposing` 检查，防止中文输入法 Enter 键误触发
- 使用 `filter({ user_email })` 代替 `list()` 确保用户隔离

**后端** (`functions/smartChatWithSearch.ts`):
- 添加 `user.email` 验证，确保不为空
- 统一使用 `userEmail` 变量（经过 trim 处理）
- 添加详细的查询诊断日志
...
1. **Base44 asServiceRole 的局限性**：只对 Create 绕过 RLS，Read/Update/Delete 仍受限
2. **React setState 异步问题**：高频操作需要 useRef 配合
3. **IME 输入法处理**：中文输入需检查 `e.isComposing`
4. **RLS 需要逐项测试**：不同操作可能有不同行为

### 代码变更

| 文件 | 变更行数 |
|------|----------|
| `useChatState.jsx` | +30 行 |
| `smartChatWithSearch.ts` | +25 行 |

### 相关提交

- `8ee8624` - fix: 前端添加 user_email 过滤确保对话隔离
- `6f876f1` - fix: 添加对话查询诊断日志和备用查询方案
- `e5ddc36` - fix: 添加 IME 输入法 isComposing 检查
- `855db51` - fix: 方案 B - 修复后端 user_email 验证和写入问题
- `a182f90` - fix: 添加 isStreamingRef 防止重复发送请求

---

## 2026-01-12 (对话窗口隔离性失效修复) 🔧
...
位置: `src/components/hooks/useChatState.jsx`
...
使用 `useRef` 同步跟踪 conversationId，解决异步状态更新竞态条件。

### 代码变更

**文件**: `src/components/hooks/useChatState.jsx`

| 修改点 | 行号 | 内容 |
|--------|------|------|
| 添加 ref | 90 | `conversationIdRef = useRef(null)` |
| 新建对话重置 | 184 | `conversationIdRef.current = null` |
| 选择对话更新 | 204 | `conversationIdRef.current = conv.id` |
| 发送消息使用 ref | 330 | `currentConvId = conversationIdRef.current \|\| currentConversation?.id` |
| 响应后更新 ref | 368 | `conversationIdRef.current = convId` |
| 自动发送更新 ref | 644 | `conversationIdRef.current = responseData.conversation_id` |

### 架构健康度评分

| 维度 | 得分 |
|------|------|
| conversationId 生成逻辑 | 8/10 |
| conversationId 传递路径 | 5/10 → 9/10 |
| 状态管理 | 4/10 → 8/10 |
| 总分 | 60/100 → 80/100 |
...
修复后需验证:
- [ ] 单个对话中所有消息共享同一 conversationId
- [ ] 新建对话创建新的 conversationId
- [ ] 不同对话完全隔离，无消息混淆
- [ ] 对话列表正确显示所有对话
- [ ] 切换对话时正确加载历史消息
...
详见: `diagnosis/conversation-isolation/root-cause-analysis.md`
