...
**功能模块跳转即时反馈** - 已完全修复
- 问题：点击功能模块跳转后无法实时显示对话状态，需等待 AI 响应或刷新页面
- 原因：useEffect 在渲染后执行，无法提供首次渲染时的即时反馈
- 解决：useState 初始化函数 + useMemo 同步读取 sessionStorage
- 详见：TROUBLESHOOTING.md 和 CHANGELOG.md

**对话窗口隔离性失效** - 已完全修复
- 问题：每轮对话创建新记录、重复发送、新对话不显示
- 原因：React 异步状态 + IME 输入法 + Base44 RLS 限制
- 详见：TROUBLESHOOTING.md 和 CHANGELOG.md
...
| 问题 | 位置 | 状态 |
|------|------|------|
| 文档整合 | 旧文档与 .claude/ 系统 | ✅ 已整合到 archive/ |
